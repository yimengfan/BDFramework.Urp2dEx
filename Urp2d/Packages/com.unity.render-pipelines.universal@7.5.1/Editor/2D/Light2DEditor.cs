    ...
    internal class Light2DEditor : PathComponentEditor<ScriptablePath>
    {
        ...
        class FreeformShapeTool : PathEditorTool<ScriptablePath>
        {
            ...
            protected override void SetShape(ScriptablePath shapeEditor, SerializedObject serializedObject)
            {
                ...
                for (var i = 0; i < shapeEditor.pointCount; ++i) pointsProperty.GetArrayElementAtIndex(i).vector3Value = shapeEditor.GetPoint(i).position;
        ...
        private static class Styles
        {
            ...
            public static GUIContent generalIsMainLight             = EditorGUIUtility.TrTextContent("主光源", "是否为主光源");
            public static GUIContent generalLightDirection             = EditorGUIUtility.TrTextContent("方向", "全局光的方向");
        ...
        SerializedProperty m_IsMainLight;
        ...
        private void AnalyticsTrackChanges(SerializedObject serializedObject)
        {
            if (serializedObject.hasModifiedProperties)
            {
                foreach (Object targetObj in serializedObject.targetObjects)
                {
                    ...
                    if (!m_ModifiedLights.Contains(light2d)) m_ModifiedLights.Add(light2d);
                }
            }
        }
        ...
        void OnEnable()
        {
            ...
            m_IsMainLight = serializedObject.FindProperty("m_IsMainLight");
        ...
        void OnPointLight(SerializedObject serializedObject)
        {
            ...
            if (EditorGUI.EndChangeCheck()) m_PointInnerAngle.floatValue = Mathf.Min(m_PointInnerAngle.floatValue, m_PointOuterAngle.floatValue);

            EditorGUI.BeginChangeCheck();
            EditorGUILayout.Slider(m_PointOuterAngle, 0, 360, Styles.pointLightOuterAngle);
            if (EditorGUI.EndChangeCheck()) m_PointOuterAngle.floatValue = Mathf.Max(m_PointInnerAngle.floatValue, m_PointOuterAngle.floatValue);

            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(m_PointInnerRadius, Styles.pointLightInnerRadius);
            if (EditorGUI.EndChangeCheck()) m_PointInnerRadius.floatValue = Mathf.Max(0.0f, Mathf.Min(m_PointInnerRadius.floatValue, m_PointOuterRadius.floatValue));

            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(m_PointOuterRadius, Styles.pointLightOuterRadius);
            if (EditorGUI.EndChangeCheck()) m_PointOuterRadius.floatValue = Mathf.Max(m_PointInnerRadius.floatValue, m_PointOuterRadius.floatValue);
        ...
        void OnShapeLight(Light2D.LightType lightType, SerializedObject serializedObject)
        {
            ...
            else if (lightType == Light2D.LightType.Parametric || lightType == Light2D.LightType.Freeform)
            {
                if (lightType == Light2D.LightType.Parametric)
                {
                    ...
                    if (m_ShapeLightParametricRadius.floatValue < 0) m_ShapeLightParametricRadius.floatValue = 0;
                ...
                if (m_ShapeLightFalloffSize.floatValue < 0) m_ShapeLightFalloffSize.floatValue = 0;
        ...
        Vector3 DrawAngleSlider2D(Transform transform, Quaternion rotation, float radius, float offset, Handles.CapFunction capFunc, float capSize, bool leftAngle, bool drawLine, bool useCapOffset, ref float angle)
        {
            ...
            if (drawLine) Handles.DrawLine(transform.position, cwHandle);
        ...
        private void DrawAngleHandles(Light2D light)
        {
            ...
            if (diff != 0.0f) light.pointLightInnerAngle = Mathf.Max(0.0f, light.pointLightInnerAngle + diff);
            ...
            if (diff != 0.0f) light.pointLightInnerAngle = light.pointLightInnerAngle < light.pointLightOuterAngle ? light.pointLightInnerAngle : light.pointLightOuterAngle;
        ...
        void OnSceneGUI()
        {
            ...
            switch (light.lightType)
            {
                case Light2D.LightType.Point:
                {
                    ...
                    if (GUI.changed) EditorUtility.SetDirty(light);
                }
                    break;
                ...
                case Light2D.LightType.Parametric:
                {
                    ...
                    if (sides == 4) angleOffset = Mathf.PI / 4.0f + Mathf.Deg2Rad * light.shapeLightParametricAngleOffset;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorGUI.EndChangeCheck()) m_LightType.intValue = newLightType;
            ...
            {
                EditorGUILayout.PropertyField(m_LightDirection, Styles.generalLightDirection);
            }

            //主光源
            if (m_LightType.intValue != (int) Light2D.LightType.Global  )
            {
                if (m_IsMainLight.boolValue)
                {
                    GUI.color = Color.red;
                }
                
                EditorGUILayout.PropertyField(m_IsMainLight, Styles.generalIsMainLight);
                ...
                if (m_IsMainLight.boolValue)
                {
                    ...
                    var rets   = lights.Where((l) => l.IsMainLight == true || l.lightOrder==0);
                    foreach (var light2D in rets)
                    {
                        if (light2D.GetInstanceID() != (this.target as Light2D).GetInstanceID())
                        {
                            light2D.IsMainLight = false;
                            light2D.lightOrder  = 1;
                        }
                        else
                        {
                            m_LightOrder.intValue = 0;
                        }
                    }
                }
                ...
                // if (rets.Count() > 1)
                // {
                //     var name = "主光源只能有一个，请检查:";
                //     foreach (var light2D in rets)
                //     {
                //         name += ("-" + light2D.gameObject.name);
                //     }
                //
                //     EditorGUILayout.HelpBox(name, MessageType.Error);
                // }
            }

            if (m_LightType.intValue != (int) Light2D.LightType.Global) EditorGUILayout.PropertyField(m_AlphaBlendOnOverlap, Styles.generalLightOverlapMode);
            ...
            if (EditorGUI.EndChangeCheck()) m_LightIntensity.floatValue = Mathf.Max(m_LightIntensity.floatValue, 0);
            ...
            if (m_LightType.intValue != (int) Light2D.LightType.Global)
            {
                ...
                if (m_UseNormalMap.boolValue)
                {
                    ...
                    if (EditorGUI.EndChangeCheck()) m_PointZDistance.floatValue = Mathf.Max(0.0f, m_PointZDistance.floatValue);
                ...
                if (m_VolumetricAlpha.floatValue > 0) EditorGUILayout.Slider(m_ShadowVolumeIntensity, 0, 1, Styles.generalShadowVolumeIntensity);
            }
