...
namespace UnityEngine.Experimental.Rendering.Universal
{
    internal static class RendererLighting
    {
        ...
        static readonly string[] k_UseBlendStyleKeywords = {"USE_SHAPE_LIGHT_TYPE_0", "USE_SHAPE_LIGHT_TYPE_1", "USE_SHAPE_LIGHT_TYPE_2", "USE_SHAPE_LIGHT_TYPE_3"};

        static readonly string[] k_BlendFactorsPropNames = {"_ShapeLightBlendFactors0", "_ShapeLightBlendFactors1", "_ShapeLightBlendFactors2", "_ShapeLightBlendFactors3"};

        static readonly string[] k_MaskFilterPropNames = {"_ShapeLightMaskFilter0", "_ShapeLightMaskFilter1", "_ShapeLightMaskFilter2", "_ShapeLightMaskFilter3"};

        static readonly string[] k_InvertedFilterPropNames = {"_ShapeLightInvertedFilter0", "_ShapeLightInvertedFilter1", "_ShapeLightInvertedFilter2", "_ShapeLightInvertedFilter3"};
        ...
        static public void Setup(RenderingData renderingData, Renderer2DData renderer2DData)
        {
            ...
            if (s_NormalsTarget.id == 0) s_NormalsTarget.Init("_NormalMap");

            if (s_ShadowsRenderTarget.id == 0) s_ShadowsRenderTarget.Init("_ShadowTex");
            ...
            if (s_LightMaterials == null) s_LightMaterials = new Material[k_NumberOfLightMaterials];

            // This really needs to be deleted and replaced with a material block
            const int totalMaterials                         = 256;
            if (s_ShadowMaterials == null) s_ShadowMaterials = new Material[totalMaterials];

            if (s_RemoveSelfShadowMaterials == null) s_RemoveSelfShadowMaterials = new Material[totalMaterials];
        }
        ...
        static public void CreateNormalMapRenderTexture(CommandBuffer cmd)
        {
            if (!s_HasSetupRenderTextureFormatToUse)
            {
                ...
                else if (SystemInfo.IsFormatSupported(GraphicsFormat.R16G16B16A16_SFloat, FormatUsage.Linear | FormatUsage.Render)) s_RenderTextureFormatToUse = GraphicsFormat.R16G16B16A16_SFloat;
                    ...
                    for (int group = 0; group < shadowCasterGroups.Count; group++)
                    {
                        ...
                        if (shadowCasters != null)
                        {
                            ...
                            for (int i = 0; i < shadowCasters.Count; i++)
                            {
                                ...
                                {
                                    if (shadowCaster.castsShadows) cmdBuffer.DrawMesh(shadowCaster.mesh, shadowCaster.transform.localToWorldMatrix, shadowMaterial);
                                }
                            }
                        ...
                        if (light != null && light.lightType != Light2D.LightType.Global && light.volumeOpacity > 0.0f && light.blendStyleIndex == blendStyleIndex && light.IsLitLayer(layerToRender) && light.IsLightVisible(camera))
                        {
                            ...
                            if (lightVolumeMaterial != null)
                            {
                                ...
                                if (lightMesh != null)
                                {
                                    ...
                                    if (light.lightType == Light2D.LightType.Sprite && light.lightCookieSprite != null && light.lightCookieSprite.texture != null) cmdBuffer.SetGlobalTexture("_CookieTex", light.lightCookieSprite.texture);
        ...
        static public void SetShapeLightShaderGlobals(CommandBuffer cmdBuffer)
        {
            for (int i = 0; i < s_BlendStyles.Length; ++i)
            {
                if (i >= k_BlendFactorsPropNames.Length) break;
        ...
        {
            if (s_FalloffLookupTexture == null) s_FalloffLookupTexture = Light2DLookupTexture.CreateFalloffLookupTexture();
        ...
        static public void SetPointLightShaderGlobals(CommandBuffer cmdBuffer, Light2D light)
        {
            ...
            {
                cmdBuffer.SetGlobalVector("_2DMainLightPosition", light.transform.position);
                cmdBuffer.SetGlobalFloat("_2DMainLightZDistance", light.pointLightDistance);
            }
            ...
            if (light.lightCookieSprite != null && light.lightCookieSprite.texture != null) cmdBuffer.SetGlobalTexture("_PointLightCookieTex", light.lightCookieSprite.texture);
        }
        ...
        static public void ClearDirtyLighting(CommandBuffer cmdBuffer, uint blendStylesUsed)
        {
            for (int i = 0; i < s_BlendStyles.Length; ++i)
            {
                if ((blendStylesUsed & (uint) (1 << i)) == 0) continue;
        ...
        static Material CreateLightMaterial(Light2D light, bool isVolume)
        {
            ...
            if (light.lightType == Light2D.LightType.Sprite) material.EnableKeyword(k_SpriteLightKeyword);

            if (!isShape && light.lightCookieSprite != null && light.lightCookieSprite.texture != null) material.EnableKeyword(k_UsePointLightCookiesKeyword);

            if (!isShape && light.pointLightQuality == Light2D.PointLightQuality.Fast) material.EnableKeyword(k_LightQualityFastKeyword);

            if (light.useNormalMap) material.EnableKeyword(k_UseNormalMap);
        ...
        static Material GetLightMaterial(Light2D light, bool isVolume)
        {
            ...
            if (s_LightMaterials[materialIndex] == null) s_LightMaterials[materialIndex] = CreateLightMaterial(light, isVolume);
