...
namespace UnityEngine.Experimental.Rendering.Universal
{
    internal class Renderer2D : ScriptableRenderer
    {
        ...
        //render feature
        Render2DObjectPass[] m_RenderFeaturePasses;
            ...
            out RenderTargetHandle depthTargetHandle)
        {
            ...
            if (cameraData.renderType == CameraRenderType.Base)
            {
                ...
                m_CreateDepthTexture = m_UseDepthStencilBuffer;// !cameraData.resolveFinalTarget && m_UseDepthStencilBuffer;
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            RenderTargetHandle activeColorTargetHandle;
            RenderTargetHandle activeDepthTargetHandle;

            CommandBuffer cmd = CommandBufferPool.Get("Create Camera Textures");
            CreateRenderTextures(ref cameraData, ppcUsesOffscreenRT, colorTextureFilterMode, cmd, out activeColorTargetHandle, out activeDepthTargetHandle);
            context.ExecuteCommandBuffer(cmd);
            CommandBufferPool.Release(cmd);

            ConfigureCameraTarget(activeColorTargetHandle.Identifier(), activeDepthTargetHandle.Identifier());
            ...
            //Light Pass
            m_Render2DLightingPass.ConfigureTarget(activeColorTargetHandle.Identifier(), activeDepthTargetHandle.Identifier());
            ...
            {
                 m_CopyDepthPass.Setup(activeDepthTargetHandle, m_DepthTexture);
            ...
            if (renderingData.cameraData.requiresOpaqueTexture)
            {
                ...
                m_CopyColorPass.Setup(activeColorTargetHandle.Identifier(), m_OpaqueColor, downsamplingMethod);
                EnqueuePass(m_CopyColorPass);
            }
            
            
            //Render Feature
            for (int i = 0; i < this.m_RenderFeaturePasses.Length ; i++)
            {
                var pass = this.m_RenderFeaturePasses[i];
                if (pass != null)
                {
                    pass.ConfigureTarget(activeColorTargetHandle.Identifier(), activeDepthTargetHandle.Identifier());
                    EnqueuePass(pass);
                }
            }
            ...
            if (cameraData.postProcessEnabled)
            {
...
#endif
                {
                    m_PostProcessPass.Setup(
                        ...
                        activeColorTargetHandle,
                        postProcessDestHandle,
                        activeDepthTargetHandle,
                ...
                activeColorTargetHandle = postProcessDestHandle;
            }
            ...
            {
                m_FinalPostProcessPass.SetupFinalPass(activeColorTargetHandle);
                EnqueuePass(m_FinalPostProcessPass);
            }
            else if (lastCameraInStack && activeColorTargetHandle != RenderTargetHandle.CameraTarget)
            {
                m_FinalBlitPass.Setup(cameraTargetDescriptor, activeColorTargetHandle);
